...
#define MOTOR_PWM_OUTPUT_LA_PIN 27u
#define MOTOR_PWM_OUTPUT_LB_PIN 14u
#define MOTOR_PWM_OUTPUT_RA_PIN 18u
#define MOTOR_PWM_OUTPUT_RB_PIN 13u
#define ENCODER_INPUT_LA_PIN 34u
#define ENCODER_INPUT_LB_PIN 35u
#define ENCODER_INPUT_RA_PIN 36u
#define ENCODER_INPUT_RB_PIN 39u
...
static esp_err_t motor_R_init(void)
{
    ...
    ret = mcpwm_gpio_init(MCPWM_UNIT_1, MCPWM0A, MOTOR_PWM_OUTPUT_RA_PIN);
    if(ret != ESP_OK)
    {   
    ...
    ret = mcpwm_set_duty(MCPWM_UNIT_1, MCPWM_TIMER_0, MCPWM_OPR_A, 0.0f);
    if(ret != ESP_OK)
    {   
    ...
    ret = mcpwm_gpio_init(MCPWM_UNIT_1, MCPWM0B, MOTOR_PWM_OUTPUT_RB_PIN);
    if(ret != ESP_OK)
    {   
    ...
    ret = mcpwm_set_duty(MCPWM_UNIT_1, MCPWM_TIMER_0, MCPWM_OPR_B, 0.0f);
    if(ret != ESP_OK)
    {   
